...
export default function useIrismodToken() {
  const client = useClient();
  const QueryToken = (denom: string,  options: any) => {
    const key = { type: 'QueryToken',  denom };    
    return useQuery([key], () => {
      const { denom } = key
      return  client.IrismodToken.query.queryToken(denom).then( res => res.data );
    }, options);
  }
  ...
  const QueryTokens = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery([key], ({pageParam = 1}: { pageParam?: number}) => {
      ...
      return  client.IrismodToken.query.queryTokens(query ?? undefined).then( res => ({...res.data,pageParam}) );
    }, {...options,
  ...
  const QueryFees = (symbol: string,  options: any) => {
    ...
    return useQuery([key], () => {
      const { symbol } = key
      return  client.IrismodToken.query.queryFees(symbol).then( res => res.data );
    }, options);
  }
  ...
  const QueryParams = ( options: any, perPage: number) => {
    ...
    return useInfiniteQuery([key], ({pageParam = 1}: { pageParam?: number}) => {
      ...
      query['pagination.offset']= (pageParam-1)*perPage;
      query['pagination.count_total']= true;
      return  client.IrismodToken.query.queryParams().then( res => ({...res.data,pageParam}) );
    }, {...options,
      getNextPageParam: (lastPage, allPages) => { if ((lastPage.pagination?.total ?? 0) >((lastPage.pageParam ?? 0) * perPage)) {return lastPage.pageParam+1 } else {return undefined}},
      getPreviousPageParam: (firstPage, allPages) => { if (firstPage.pageParam==1) { return undefined } else { return firstPage.pageParam-1}}
    }
  ...
  const QueryTotalBurn = ( options: any) => {
    ...
    return useQuery([key], () => {
      return  client.IrismodToken.query.queryTotalBurn().then( res => res.data );
    }, options);
  }
