...
export default function usePetrimodCoinswap() {
  const client = useClient();
  const QueryLiquidityPool = (lpt_denom: string,  options: any) => {
    const key = { type: 'QueryLiquidityPool',  lpt_denom };    
    return useQuery([key], () => {
      const { lpt_denom } = key
      return  client.PetrimodCoinswap.query.queryLiquidityPool(lpt_denom).then( res => res.data );
    }, options);
  }
  ...
  const QueryLiquidityPools = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery([key], ({pageParam = 1}: { pageParam?: number}) => {
      ...
      return  client.PetrimodCoinswap.query.queryLiquidityPools(query ?? undefined).then( res => ({...res.data,pageParam}) );
    }, {...options,
