...
export default function useIrismodRandom() {
  const client = useClient();
  const QueryRandom = (req_id: string,  options: any) => {
    const key = { type: 'QueryRandom',  req_id };    
    return useQuery([key], () => {
      const { req_id } = key
      return  client.IrismodRandom.query.queryRandom(req_id).then( res => res.data );
    }, options);
  }
  ...
  const QueryRandomRequestQueue = (query: any, options: any) => {
    ...
    return useQuery([key], () => {
      const {query } = key
      return  client.IrismodRandom.query.queryRandomRequestQueue(query ?? undefined).then( res => res.data );
    }, options);
  }
