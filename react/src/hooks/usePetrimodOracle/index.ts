...
export default function usePetrimodOracle() {
  const client = useClient();
  const QueryFeed = (feed_name: string,  options: any) => {
    const key = { type: 'QueryFeed',  feed_name };    
    return useQuery([key], () => {
      const { feed_name } = key
      return  client.PetrimodOracle.query.queryFeed(feed_name).then( res => res.data );
    }, options);
  }
  ...
  const QueryFeeds = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery([key], ({pageParam = 1}: { pageParam?: number}) => {
      ...
      return  client.PetrimodOracle.query.queryFeeds(query ?? undefined).then( res => ({...res.data,pageParam}) );
    }, {...options,
  ...
  const QueryFeedValue = (feed_name: string,  options: any) => {
    ...
    return useQuery([key], () => {
      const { feed_name } = key
      return  client.PetrimodOracle.query.queryFeedValue(feed_name).then( res => res.data );
    }, options);
  }
