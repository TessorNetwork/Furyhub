...
testnet-init:
	@if ! [ -f build/nodecluster/node0/petri/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/home petri-labs/petrihub petri testnet --v 4 --output-dir /home/nodecluster --chain-id petrihub-test --keyring-backend test --starting-ip-address 192.168.10.2 ; fi
