...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./lite/statik/statik.go" -not -path "*.pb.go" | xargs goimports -w -local github.com/gridironprotocol/gridiron
...
testnet-init:
	@if ! [ -f build/nodecluster/node0/grid/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/home nimrostafarian/gridironprotocol:grid.1                                                                                                                                            grid testnet --v 4 --output-dir /home/nodecluster --chain-id GridIron-test --keyring-backend test --starting-ip-address 192.168.10.2 ; fi
