...
Any Grid holder, whether bonded or unbonded, can submit proposals by sending a
...
grid tx gov submit-proposal param-change proposal.json --from=<key-name> --fees=0.3grid --chain-id=GridIron
```
...
grid tx gov submit-proposal community-pool-spend proposal.json --from=<key-name> --fees=0.3grid --chain-id=GridIron
```
