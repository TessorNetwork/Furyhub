...
```bash
petri tx gov submit-proposal software-upgrade <plan-name> \
  --deposit <deposit> \
...
```bash
petri tx gov deposit <proposal-id> <deposit> --from=<from> --chain-id=<chain-id> --fees=0.3petri -b block -y
```
...
```bash
petri tx gov vote <proposal-id> <option> --from=<from> --chain-id=<chain-id> --fees=0.3petri -b block -y
```
