...
nft metadata (`metadata`) can be stored directly on the chain, or the `URI` of its storage source outside the chain can be stored on the chain. nft metadata is organized according to a specific [JSON Schema](https://JSON-Schema.org/). [Here](https://github.com/tessornetwork/furyhub/blob/master/docs/features/nft-metadata.json) is an example of metadata JSON Schema.
